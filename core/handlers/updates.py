...
def content_user(message: Message):
    ...
    logger.info(f"сообщение от пользователя: uid: {userID} | username: {userNAME} | [{message.text if message.content_type == "text" else message.content_type}]")
    userOBJ.exit()
...
def sys_message_clean(message: Message):
    logger.debug(f"content_type_services: {message.content_type}")
    try:
